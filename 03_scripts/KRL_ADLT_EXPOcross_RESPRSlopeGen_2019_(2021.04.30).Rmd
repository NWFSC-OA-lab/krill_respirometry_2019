...
dResp90 <- read_csv(here::here("02_processedData","KRL_ADLT_EXPOcross_RespirometryDataframe_Summer2019_2021.04.30.csv"))
...
### 11.2) Removing Positive Slopes
```{r}

dSlopes <- droplevels(filter(dSlopes, slope < 0.0))

slopeplot2 <- ggplot(dSlopes, aes(Treatment, slope)) +
            geom_jitter(color = "grey") +
            geom_boxplot(notch = TRUE, outlier.shape = NA, colour = "blue") +
            xlab("Treatment") +
            ylab("Oxygen Consumption") +
...
slopeplot3 <- ggplot(dSlopes, aes(Treatment, CorrWeightSlope)) +
            geom_jitter(color = "grey") +
            geom_boxplot(notch = TRUE, outlier.shape = NA, colour = "black") +
            # facet_wrap(~TrialID) +
            xlab("Treatment") +
            ylab("µg O2 h-1 g-1") +
            ggtitle("Corrected Oxygen Consumption Slopes by Treatment (Vial Oxygen (µg)/hr per gram of Krill)") +
            theme_bw()

slopeplot3
slopeplot3 + theme(legend.position="bottom")
slopeplot3 + theme(legend.position="bottom", plot.title = element_text(hjust = 0.5))
...
write.csv(dSlopes.summary, "KRL_ADLT_EXPOcross_dSlopeSummaryStats_2021.03.30.csv")
```
...
dSlopes$Treatment <- fct_relevel(dSlopes$Treatment, "CHG")
lmer.dSlopes <- lmer(CorrWeightSlope ~ Treatment + (1|MOATS), data = dSlopes)
summary(lmer.dSlopes)
...
```{r 29.a All Points}
write.csv(dSlopes, file = "KRL_ADLT_exposure_dSlopes_2021.04.30.csv", row.names = FALSE)
```
